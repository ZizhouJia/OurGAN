...
@click.option('--learning_rate',default=[0.001,0.001,0.001,0.001],nargs=4,type=float,help="the learning_rate of the four optimizer")
...
def train(batch_size,epoch,dataset_name,model_name,learning_rate,reconst_param,feature_d_loss_param,model_save_path):
    is_cuda=False
    ...
    for i in range(0,epoch):
        ...
        for step,(x1,x2) in enumerate(mnist_loader):
            ...
            #最后更新Generator
            total_loss=reconst_loss*reconst_param+G_image_loss+G_feature_loss*feature_d_loss_param
            total_loss.backward()
